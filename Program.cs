...
namespace MazeSolver
{
	internal class Program
	{
		...
		static bool IsMatchingColor(Color color1, Color color2) => color1.R == color2.R && color1.G == color2.G && color1.B == color2.B;

		// converts pixel to 0-based cell number
		static int PixelToCell(int pixel) => pixel / _cellWidth;
		// converts 0-based cell number to pixel
		static int CellToPixel(int cell) => cell * _cellWidth + _cellWidth / 2;

		static void DisplayUsage() => Console.WriteLine("USAGE:  MazeSolver <InputImageFile(JPG/PNG/BMP)> <OutputImageFile>");
		...
		static void Main(string[] args)
		{
			...
			{
#if DEBUG
				var inFile = AppDomain.CurrentDomain.BaseDirectory + @"..\..\Maze2.png";
			...
			if (startX < 0)
			{
				Console.WriteLine("ERROR: No starting point was found!");
				return;
			}
			Debug.WriteLine($"StartPoint = {startX}/{startY}, Width = {_cellWidth}");

			// allocate matrix to track which cells have been tried
		...
		// return true if a border exists between cells that are side by side
		...
		// return true if a border exists between cells that are above/below each other
